...
if __name__ == "__main__":
    ...
    new_game_count = [50, 50, 50, 50, 50, 50, 50, 50]
    row = 0
    
    game_list = list(pd.read_csv(config["match_list_file"],sep='\t',header=None).loc[:,0])
    summonernames_used = list(pd.read_csv(config["summoner_used_file"],sep='\t',header=None).loc[:,0])

    summoner_df.index = summoner_df['summonerName']
    summoner_df = summoner_df.drop(summonernames_used)
    
    try:
        while np.sum(new_game_count) > 16 and row < 10000:
        # while row < 80000:
