...
if __name__ == "__main__":
    ...
    matches_used = list(pd.read_csv(config['matches_used_file'],sep='\t',header=None).loc[:,0])
    
    game_list = pd.read_csv(config['match_list_file'],sep='\t',header=None)
    game_list.index = game_list.loc[:,0]
    game_list = list(game_list.drop(matches_used).loc[:,0])    
    
    game_info = pd.read_csv(config['match_history_file'],sep='\t')
    row=0
    ...
    try:
        while row < 100000:
            ...
            if out is None or out['info']['endOfGameResult'].split('_')[0] == 'Abort':
                matches_used.append(game_list[row])
                row += 1
                continue
            else:
                game_info = pd.concat([game_info, 
                    pd.DataFrame([[game_list[row]] + \
        ...
        game_info.to_csv(config['match_history_file'], sep='\t',index=None)